...
public class destroy_block_red : MonoBehaviour {

	private static float fallspeed = 1.5f;
	static int pausenum = OnButtonTouch.pausenum;
	public static int block_lives_red;
	public static int block_score_red;
//	public GameObject Red_Sparks;
	public GameObject Purple_Sparks;
	public static int level; //public for "Score!" graphic later
	...
	void Start () {
		transform.position = new Vector2(Random.Range(-3f,3f), Random.Range(9f,26f));
		fallspeed = 1f;
		block_lives_red = 0;
		block_score_red = 0;
	}
	...
	//Upong collision with other object
	//''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	...
	void OnCollisionEnter2D(Collision2D other){
		...
		if (other.gameObject.CompareTag("Ball_Red")){
			block_score_red = (block_score_red + 1*level); //Implement "level" value for scoring
